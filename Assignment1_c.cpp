...
// Function to multiply two matrices serially

vector<vector<int>> multiply_matrices_serial(vector<vector<int>> &A, vector<vector<int>> &B, int num_threads){
    int N = A.size();
...
int main(){
    ...
    vector<vector<int>> C = multiply_matrices_serial(A, B, num_threads);
    double end_time = omp_get_wtime();

    double serial_time = end_time - start_time;
    cout << "Time taken for serial matrix multiplication: " << serial_time << " seconds" << endl;
    
    start_time = omp_get_wtime();
    vector<vector<int>> D = multiply_matrices_parallel(A, B, num_threads);
